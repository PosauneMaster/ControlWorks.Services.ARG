...
{
    public class TekniplexVariableService
    {
        ...
        private readonly Dictionary<string, int> _variableStateLookup = new Dictionary<string, int>();

        public TekniplexVariableService()
        {
            MAX_SENSOR_DATA = Common.ConfigurationProvider.MaxSensorData;
        ...
        public void LoadVariables(Cpu cpu)
        {
            ...
            {
                CreateVariable(cpu, $"CoilData.Data[{i}].Position");
                CreateVariable(cpu, $"CoilData.Data[{i}].SensorData[0]");
                CreateVariable(cpu, $"CoilData.Data[{i}].SensorData[1]");
                CreateVariable(cpu, $"CoilData.Data[{i}].SensorData[2]");
                CreateVariable(cpu, $"CoilData.Data[{i}].SensorData[3]");
                CreateVariable(cpu, $"CoilData.Data[{i}].SensorData[4]");
            }
        ...
        private void OnVariableChanged(Variable variable)
        {
            ...
            if (variable.Name == "btnSendProductionData")
            {
                ...
                if (btnSendProductionData)
                {
                    ...
                    Trace.TraceInformation("Set btnSendProductionData to false  Processing data complete");
                }
            }
        }
